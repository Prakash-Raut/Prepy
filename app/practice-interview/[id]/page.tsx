...
import { ChevronDown, Clock, Mic2, Share } from "lucide-react"
    ...
    <div className="min-h-screen px-24">
      ...
      <div className="container py-4">
        <div className="flex items-center gap-2 text-sm">
          <Link href="/interview-list" className="text-muted-foreground hover:underline">
            Interviews
          </Link>
          <span>/</span>
          <Link href="/interview-list" className="text-muted-foreground hover:underline">
          ...
          <div className="mt-3 relative">
            <div className="relative flex items-center justify-center bg-stone-900 rounded-xl p-1.5 sm:p-0">
              ...
              <div className="absolute bottom-4 left-4 z-10 h-7 w-7 rounded-full">
                <section className="flex h-7 w-7 items-center justify-center gap-0.5 rounded-full bg-blue-600">
                  <div className="w-1 bg-white rounded-full" style={{ height: '4px' }}></div>
                  <div className="w-1 bg-white rounded-full" style={{ height: '4px' }}></div>
                  <div className="w-1 bg-white rounded-full" style={{ height: '4px' }}></div>
                </section>
              </div>
            ...
            <div className="sm:flex mt-3 w-full gap-0 grid grid-cols-2">
              {['Microphone', 'Speakers', 'Integrated Camera'].map((device) => (
                <label key={device} className="flex w-full cursor-pointer items-center gap-1 rounded-full px-2">
                  <div className="relative inline-block w-full flex-1 text-left">
                    ...
                    >
                      <Mic2 />
                      <div className="flex-1 truncate text-ellipsis text-left font-medium">{device}</div>
                      <ChevronDown size={16} />
                    </Button>
                  </div>
                </label>
      ...
      <div className="container py-8 border-t">
        ...
        <p className="mt-4">
          ...
          <button className="ml-1 hover:underline" onClick={() => setIsExpanded(!isExpanded)}>
      ...
      <div className="container py-8 border-t">
        <div className="flex items-center justify-between mb-6">
          ...
          <Link href="/interview-list" className="text-blue-500 hover:underline">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {similarInterviews.map((interview) => (
            <Link href={`/practice-interview/${interview.id}`} key={interview.id} className="block">
              <div className="rounded-xl overflow-hidden h-full flex flex-col">
                <div className={`${interview.backgroundColor} p-8 flex items-center justify-center`}>
                  <div className="h-16 w-16 bg-white rounded-full flex items-center justify-center shadow-md">
                    <span className="text-indigo-500 font-bold text-2xl">M</span>
                  </div>
                </div>
                <div className="p-4 border border-t-0 rounded-b-xl flex-1 flex flex-col">
                  ...
                  <div className="mt-auto pt-4 flex items-center gap-4">
                    ...
                    <Button size="sm" className="text-sm">{interview.difficulty}</Button>
                  </div>
                </div>
              </div>
            </Link>
